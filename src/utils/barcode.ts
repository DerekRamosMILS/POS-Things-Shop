// Minimal Code 128-B barcode renderer (SVG), no external dependencies.
const PATTERNS = [
    '212222', '222122', '222221', '121223', '121322', '131222', '122213', '122312', '132212', '221213',
    '221312', '231212', '112232', '122132', '122231', '113222', '123122', '123221', '223211', '221132',
    '221231', '213212', '223112', '312131', '311222', '321122', '321221', '312212', '322112', '322211',
    '212123', '212321', '232121', '111323', '131123', '131321', '112313', '132113', '132311', '211313',
    '231113', '231311', '112133', '112331', '132131', '113123', '113321', '133121', '313121', '211331',
    '231131', '213113', '213311', '213131', '311123', '311321', '331121', '312113', '312311', '332111',
    '314111', '221411', '431111', '111224', '111422', '121124', '121421', '141122', '141221', '112214',
    '112412', '122114', '122411', '142112', '142211', '241211', '221114', '413111', '241112', '134111',
    '111242', '121142', '121241', '114212', '124112', '124211', '411212', '421112', '421211', '212141',
    '214121', '412121', '111143', '111341', '131141', '114113', '114311', '411113', '411311', '113141',
    '114131', '311141', '411131', '211412', '211214', '211232', '2331112',
];

/** Render `data` (printable ASCII) as a Code 128-B barcode SVG string. */
export function code128SVG(data: string, opts?: { module?: number; height?: number }): string {
    const module = opts?.module ?? 2;
    const height = opts?.height ?? 48;
    const chars = [...data].filter(c => {
        const n = c.charCodeAt(0);
        return n >= 32 && n <= 126;
    });
    if (chars.length === 0) return '';

    const values: number[] = [104]; // Start B
    let sum = 104;
    for (let i = 0; i < chars.length; i++) {
        const v = chars[i].charCodeAt(0) - 32;
        values.push(v);
        sum += v * (i + 1);
    }
    values.push(sum % 103); // checksum
    values.push(106); // Stop

    let x = 0;
    const rects: string[] = [];
    for (const v of values) {
        const pat = PATTERNS[v];
        for (let i = 0; i < pat.length; i++) {
            const w = parseInt(pat[i], 10) * module;
            if (i % 2 === 0) rects.push(`<rect x="${x}" y="0" width="${w}" height="${height}" fill="#000"/>`);
            x += w;
        }
    }
    return `<svg xmlns="http://www.w3.org/2000/svg" width="${x}" height="${height}" viewBox="0 0 ${x} ${height}">${rects.join('')}</svg>`;
}
